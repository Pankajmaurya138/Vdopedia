                    ...
                    <div class="row secBg">
                        <div class="large-12 columns inner-flex-video">
                            <div class="flex-video widescreen">
                                <video id="video_play_id{{ $getVideoInfo->id}}" width="770px" height="480px" alt="{{$getVideoInfo->title}}" title="{{$getVideoInfo->title}}" class="video-js vjs-default-skin video_palyerclass" controls preload="auto" data-setup='{ "asdf": true }'>
                                <source src="{{asset('storage')}}/{{ $getVideoInfo->video_file }}" alt="{{$getVideoInfo->title}}" title="{{$getVideoInfo->title}}" type="video/mp4">
                              </video>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="comment-box thumb-border">
                        <div class="media-object stack-for-small">
                            <div class="media-object-section comment-img text-center">
                                ...
                                <h2><a href="{{ isset(Auth::user()->id) ? url('/profile/'.base64_encode(Auth::user()->id)):'' }}"> {{isset(Auth::user()->id)?ucfirst(Auth::user()->name ):''}}</a></h2>
                            </div>
...
<script type="text/JavaScript" id="playfile" class="content_reload">
   ...
   // videojs.getPlayer('myPlayer2ID').ready(function() {
        // When the player is ready, get a reference to it
        myPlayer2 =   videojs('video_play_id{{ $getVideoInfo->id }}',{controls: true,autoplay: false,fluid:true});
        var aspectRatio = 315/640; 

        function resizeVideoJS(){
            var width = document.getElementById('video_play_id{{ $getVideoInfo->id }}').parentElement.offsetWidth;
            myPlayer2.width(width);
            myPlayer2.height( width * aspectRatio );
      ...
      ?>
        ...
        if(category_id == 2 || category_id == 4) {
            ...
            //myPlayer2.addClass("hide-overlay");
            ...
            // Hide the overlay
                ...
                // Show the overlay
                overlayDisplayed = true;
                myPlayer2.removeClass("hide-overlay");
            }
        });
    }else{
...
<script type="text/javascript" >
    var obj = {
            ...
            categoryWiseFilter:function(data) {
               ...
               // videojs.getPlayer('myPlayer2ID').ready(function() {
                    // When the player is ready, get a reference to it
                    myPlayer2 =   videojs(res.id,{controls: true,autoplay: false});
