...
<meta property="og:video" content="{{ url('/video/watch/'.base64_encode($getVideoInfo->id)) }}" />
<meta property="og:video:url" content="{{ url('/video/watch/'.base64_encode($getVideoInfo->id)) }}" />
<meta property="og:video:type" content="video/mp4" />
<meta property="og:title" content="{{ $getVideoInfo->title}}" />
<meta property="og:description"content="{{ $getVideoInfo->description}}" />
<meta property="og:image" content="{{ asset('storage') }}/{{ $getVideoInfo->image_file }}" />
            ...
            <div class="light-off-menu" id="offCanvas" data-off-canvas style="overflow-y: scroll; height:450px;">
                <ul class="vertical menu off-menu" data-responsive-menu="drilldown">
                    ...
                    <li class="has-submenu" data-dropdown-menu="example1">
                        <a href="javascript::void(0);"><i class="fa fa-th" data-toggle="tooltip" data-placement="bottom" title="category of videos"></i>category</a>
                    ...
                    <div class="row secBg">
                        <div class="large-12 columns inner-flex-video">
                            <div class="flex-video widescreen">
                                <video id="id-iframe"  width="770px"   class="video-js vjs-default-skin video_palyerclass " controls preload="auto" data-setup='{ "asdf": true }'>
                                <source src="{{asset('storage')}}/{{ $getVideoInfo->video_file }}" title="{{ $getVideoInfo->title }}" alt="{{ $getVideoInfo->title }}"  type="video/mp4">
                              </video>
                                <source src="{{asset('storage')}}/{{ $getVideoInfo->video_file }}" title="{{ $getVideoInfo->title }}" alt="{{ $getVideoInfo->title }}"  type="video/mp4" label="720P">
                                <source src="{{asset('storage')}}/{{ $getVideoInfo->video_file }}" title="{{ $getVideoInfo->title }}" alt="{{ $getVideoInfo->title }}"  type="video/mp4" label="480P" selected="true">
                                <source src="{{asset('storage')}}/{{ $getVideoInfo->video_file }}" title="{{ $getVideoInfo->title }}" alt="{{ $getVideoInfo->title }}"  type="video/mp4" label="360P">
                                <source src="{{asset('storage')}}/{{ $getVideoInfo->video_file }}" title="{{ $getVideoInfo->title }}" alt="{{ $getVideoInfo->title }}"  type="video/mp4" label="240P">
                            </div>
                        </div>
                    </div>
                </section>
                        ...
                        <div class="media-object-section object-second">
                            ...
                            <div class="social-share">
                                <div class="post-like-btn clearfix">
                                    ...
                                    <a class="secondary-button class-like css_on_single_page" data-thumb="likes" id="setColorOfLike" data-toggle="tooltip" data-placement="top" title="likes the vidoes" 
                                    ...
                                    <a  class="secondary-button class-like css_on_single_page" data-toggle="tooltip" data-placement="top" title="dislikes the vidoes"  data-thumb="dislikes" id="setColorOfDislike" 
                                    ...
                                    @if($getVideoInfo->category_id == 2 || $getVideoInfo->category_id == 4)
                                        <a class="secondary-button css_on_single_page" id="overlayButton" data-toggle="tooltip" data-placement="top" title="Lyrics on/off" ><i class="fa fa-file-audio-o"></i></a> 
                                        <a href="{{ isset($file_name) ? asset('download/lyrics_file').'/'.$file_name : '' }}
                                    " class="secondary-button css_on_single_page" data-toggle="tooltip" data-placement="top" title="download lyrics file"  download="{{ isset($file_name)? $file_name:'' }}" target="_blank" data-vid="{{ $getVideoInfo->id }}" id="download_lyrics"><i class="fa fa-download"></i></a>
                                    @else
                                   {{-- <a class="secondary-button css_on_single_page" data-toggle="tooltip" data-placement="top" title="download pdf file"><i class="fa fa-file-pdf-o"> </i></a> --}} 
                                    <a href="@if(isset($file_name)){{ asset('download'.'/'.$getVideoInfo->getCategoryName->name.'/'.$file_name) }} @endif
                                    " class="secondary-button css_on_single_page" download="{{isset( $file_name )? $file_name :''}}" target="_blank" data-vid="{{ $getVideoInfo->id }}" data-toggle="tooltip" data-placement="top" title="download pdf file"  id="download_lyrics"><i class="fa fa-download"></i></a>
                                    @endif
                                    <a href=" {{ asset('storage') }}/{{ 'mp3/'.$getVideoInfo->mp3_file }}" class="secondary-button css_on_single_page" download="{{ $getVideoInfo->mp3_file  }}" data-toggle="tooltip" data-placement="top" title="download mp3"  target="_blank"  ><i class="fa fa-music"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="media-object stack-for-small">
                            <div class="media-object-section comment-img text-center">
                                <div class="comment-box-img">
                                    <img src= "@if(!empty(Auth::user()->profile_image)) {{ asset('storage').'/'.Auth::user()->profile_image }} @else {{ asset('storage').'/images/user.png' }} @endif" alt="profile image">
                                </div>
                ...
                <div class="large-12 medium-7 medium-centered columns">
                    <div class="widgetBox">
                        ...
                        <ins class="adsbygoogle"
                             style="display:block"
                             data-ad-client="ca-pub-6588458573329944"
                             data-ad-slot="7430075958"
                             data-ad-format="auto"
                                    ...
                                    {{-- @if($getVideoInfo->id != $rVideo->id ) --}}
                                        <div class="video-box thumb-border">
                                            ...
                                            <div class="video-box-content">
                                                <h3><a href="{{ url('/video/watch/'.base64_encode($rVideo->id)) }}">{{ ucfirst($rVideo->title) }}</a></h3>
                                                <p>
...
@section('script')
   ...
   <!-- <script src="https://unpkg.com/silvermine-videojs-quality-selector/dist/js/silvermine-videojs-quality-selector.min.js"></script> -->
    ...
    $(document).ready(function(){
         var myPlayer,
        overlayDisplayed;
        myPlayer =   videojs('id-iframe',{controls: true,autoplay: false,responsive: true,fill:true});
        ...
        myPlayer.src([
           {
              src: "{{asset('storage')}}/{{ $getVideoInfo->video_file }}",
              type: 'video/mp4',
              label: '720P',
           },
           {
        ...
        function resizeVideoJS(){
            var width = document.getElementById('id-iframe').parentElement.offsetWidth;
            myPlayer.width(width);
            myPlayer.height( width * aspectRatio );

         }
         options = {
                "title": "{{isset($getVideoInfo->title)? $getVideoInfo->title:'video title'}}",
                "description": "{{isset($getVideoInfo->description)? clean($getVideoInfo->description):'video title'}}",
                "url": "{{url('video/watch/'.base64_encode($getVideoInfo->id))}}",
                "embedCode": "<iframe src='{{url('video/watch/'.base64_encode($getVideoInfo->id))}}'>",
                "offset": "{{isset($getVideoInfo->video_length)?$getVideoInfo->video_length:'00:00:00'}}",
                "services": {
                    "facebook": true,
                    "twitter": true,
                    "linkedin": true,
